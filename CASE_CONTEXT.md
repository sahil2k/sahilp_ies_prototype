...
**Self-contained screens, shared session state**
Because screens can be entered out of order, each screen has a short context line at the top establishing what state the viewer is looking at — written as real in-product UI copy (e.g. a subheading or status line a real user would see), not a meta-explanation of the demo. On a fresh visit (nothing yet done this session), every screen loads its complete default state from `/data`, so it's never broken or empty regardless of entry point.

Within a session, though, the four finance screens share one running state (`components/finance/finance-state.tsx`, kept in `sessionStorage`): promoting or demoting a task, approving or reversing the drafted entry, and confirming a handoff and closing the tax item all persist for the rest of the session, and the close-readiness score and exception counts update on every finance screen once you act, not just the one you acted on. Approving the entry on Assisted fix and navigating away and back still shows it approved; closing the tax item updates the Close dashboard's own readiness figure immediately.

Two further, narrower pieces of state are shared the same way, each with its own store: the expert handoff decision (sent / not now / not decided; `lib/handoff-store.ts`), because an expert's answer only exists if the controller asked for one — read by the Multi-state tax review screen (which handles all three states on one screen, rather than splitting "recommend" and "resolve" across two) and by the Arizona row in the exception queue; and, on the developer side, Flowcast's listing details and declared actions (`lib/agent-store.ts`), shared between Agent Studio (which edits them) and Marketplace listing (which displays them), so a customer's marketplace listing reflects what the developer actually configured. Every other developer screen (Group-Level API, API explorer, Revenue) remains fully self-contained.
...
- **State:** on a fresh visit, every screen starts from the same complete default state in `/data`. Within a session, the finance journey's own actions (approve, edit, promote, demote, close) are shared across all four finance screens and persist for the rest of the session — see the navigation model above. Two narrower pieces of state (the expert handoff decision; Flowcast's agent config) are shared the same way, each its own store.
