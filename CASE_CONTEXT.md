...
- **State:** each screen starts from the same complete mock state in `/data`. Actions on a screen (approve, edit, promote, close) update that screen only. The exception is the expert handoff decision (see the navigation model above), which is kept for the browser session so the expert's answer appears only if the controller confirmed the handoff.
...
1. **Group-Level API.** Status line: "Sandbox: Cedarline Group | 6 entities | One connection for the group". A side-by-side code comparison — "Today: one connection per entity" (looping per-entity calls, no consolidated or intercompany endpoint) against "Group-Level API: one connection for the whole group" (one call returning every entity's balance, the consolidated total, and pending intercompany transfers) — with the two real supporting figures cited by source (Intuit App Partner Program Guide; Intuit IES developer FAQ). A "What's available" list: cross-entity reads, intercompany transactions, dimensions, consolidated reports, group-level events/webhooks. Next: "Try the group-level API".
2. **API explorer.** Status line: "Sandbox: Cedarline Group | N of 6 entities connected | Connected" (starts "Not connected"). A "Connect your sandbox" panel: one entity checklist (all 6 checked by default, individually togglable) and one "Connect" button — the single-consent-screen token model from D1. Once connected, "Try a request" sends `GET /v1/groups/cedarline-group/cash-position` and returns the JSON response scoped to only the entities left checked, including Cedarline Cold Storage LLC's balance below its minimum threshold. Next: "Build an agent".
3. **Agent Studio.** Status line: "Sandbox: Cedarline Group | Flowcast | Draft". Flowcast's profile (name, tagline, category, description, $220.00 per customer per month) and its three declared actions, each with its badge and a first-person description in the AI's own voice (Automated: flags entities projected to dip below their cash minimum; Assisted: drafts a 13-week rolling forecast for review; Human handoff: hands a shortfall needing an intercompany transfer to the customer's finance team, not to an outside expert — D5's expert-network pool is roadmap-only). An "Auth and hosting" checklist (managed OAuth, multi-tenant hosting, a pre-loaded sandbox). "Run test" reveals a canned Automated result: Cedarline Cold Storage LLC flagged, balance $41,920.35 against a $50,000.00 minimum, projected in 9 days. Next: "Publish the agent".
4. **Marketplace listing.** Status line: "Flowcast | Live since April 2, 2026 | Reviewed for security". A listing preview as a customer would see it: the agent's icon, tagline, price, its three declared-action badges, a "Reviewed for security" badge, and either "Community trusted" or a ratings-so-far count. A "Trust on the marketplace" panel explains the two tiers by name (the security review checks security, not advice quality; Community trusted needs both a ratings-count threshold and a trustworthy-percentage threshold — Flowcast has 34 of the 50 ratings needed, at 91% trustworthy against an 85% bar, so it is not yet eligible). A "Liability" panel states plainly that liability for an installed agent's output sits with the customer who installed it. Next: "See revenue".
5. **Revenue.** Status line: "Flowcast | 168 active installs | Next payout $25,872.00 on October 15, 2026". Four stat tiles (active installs, this month's revenue share, lifetime revenue, developer's share of 70%), a 6-month installs-and-revenue table (April through September 2026, growing from 6 to 168 active installs), and a "Next payout" panel. Revenue is calculated per active install, split 70% developer / 30% platform fee (both illustrative). Next: "See agents in your close".
