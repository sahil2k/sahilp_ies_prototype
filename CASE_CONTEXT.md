...
| Developer screens 1–5 | `/developer/discovery` … `/developer/monetisation` | Placeholders: real title, status line and navigation, with a "Coming soon" empty state |
...
Because screens can be entered out of order, each screen has a short context line at the top establishing what state the viewer is looking at — written as real in-product UI copy (e.g. a subheading or status line a real user would see), not a meta-explanation of the demo. Screens do not assume the state left behind by a prior screen — each screen loads its own complete mock state from `/data`.
...
- **State:** each screen starts from the same complete mock state in `/data`. Actions on a screen (approve, edit, promote, confirm, close) update that screen only; nothing carries across screens, so every screen looks the same whether opened from the hub or via Next.
...
1. **Close dashboard.** Status line: "September 2026 close | Cedarline Group, 6 entities | Workday 2 of 4 | Readiness 78/100 | 15 open exceptions". Close readiness 78/100 with context (workday, last month's close of 9 workdays, exceptions, intercompany pairs eliminated 35 of 38); "What the AI did this month" by badge (4,902 / 461 / 8); readiness by entity table (Cold Storage flagged "Needs attention"); recent AI activity log where reversible actions have a Reverse button (reversed actions are struck through, including an intercompany elimination reversed by a spot check). Next: "Review exceptions".
...
4. **Expert handoff.** Heading "Multi-state tax review". Status line: "Cedarline Cold Storage LLC | Arizona tax question | Low confidence, 58% | Expert recommended, not booked" (becomes "Sent to Dana Whitfield"). Three-stage strip: Flagged (Automated) → You decide (Assisted) → Sent with context (Automated). One "Expert recommended" card holds the question, "My draft answer" at low confidence 58%, "Why I'm recommending an expert" (confidence below 70%; multi-state tax is regulated; no precedent in the group), the recommended expert (Dana Whitfield, CPA, 14 years, licensed AZ/CO/TX, 4.9 from 312 questions, $180.00 per question, within 4 business hours) with "Compare 2 experts", the price before commitment ("Illustrative price. Charged once, when you confirm."), and Confirm and send / Not now. Beside it: "What the expert receives", the packaged context, with a note that experts are independent CPAs and firms who see only this. Next: "See the expert's answer".
5. **Resolution.** Heading "Multi-state tax review". Status line: "Cedarline Cold Storage LLC | Arizona tax question | Answered by Dana Whitfield, CPA | Accrual awaiting your approval" (becomes "Closed"). The expert's answer (register for Arizona transaction privilege tax; accrue on the 29 Arizona-delivered invoices; flag future inventory moves), received in 3 hours 12 minutes; an Assisted accrual drafted from it ($4,871.25; Dr 6410 Sales and use tax expense, Cr 2235 Sales tax payable, Arizona). "Approve accrual and close item" closes the item and the readiness score counts up from 78 to **83** (Cold Storage 66 → 81). "What I learned" then shows, in the AI's voice: inventory held in a new state is now a physical-presence signal flagged for all 6 entities; tax questions still go to an expert because the topic is regulated, but with a stronger draft. Next: "See agents in your close".

### Developer screens (planned; currently placeholders)

1. Discovery — developer portal: "one connection per entity" → "one connection for the whole group".
2. Onboarding — Group-Level API explorer with a sample cross-entity request/response; one-click multi-entity sandbox; a single "Connect" consent with per-entity scopes (see D1 token model).
3. Building — Agent Studio: configure an agent, declare Automated / Assisted / Handoff actions (descriptive text, see D2); auth and hosting handled.
4. Publishing — in-product marketplace listing preview with "Reviewed for security" plus the separate "Community trusted" badge.
5. Monetisation — revenue dashboard: active installs, revenue share per active install, payout.

Until built, each developer screen shows its title, a status line ("Sandbox: Cedarline Group | 6 entities"), its one-line description and a "Coming soon" empty state.

**Connecting screen (reachable from both hubs; full version planned):** the developer's published agent running inside the finance leader's close, its actions carrying the same three badges as Intuit's own agents. Message: one trust pattern across Intuit's and third-party agents.
